...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const ev = await dbGet("nf_events");
      if (ev) setEventsState(ev);
      const m = await dbGet("nf_members");
      if (m && m.length > 0) {
        // INITIAL_DATA 기준으로 graduateDate 등 최신 필드 병합
        const initialMap = {};
        INITIAL_DATA.forEach(x => { initialMap[x.name] = x; });
        const merged = m.map(x => initialMap[x.name]
          ? { ...initialMap[x.name], ...x, graduateDate: initialMap[x.name].graduateDate || x.graduateDate || "" }
          : x
        );
        const existingNames = new Set(m.map(x => x.name));
        const toAdd = INITIAL_DATA.filter(x => !existingNames.has(x.name));
        const final = [...merged, ...toAdd];
        setMembersState(final);
        dbSet("nf_members", final);
      } else {
        setMembersState(INITIAL_DATA);
        dbSet("nf_members", INITIAL_DATA);
      }
      setReady(true);
    })();
  }, []);
